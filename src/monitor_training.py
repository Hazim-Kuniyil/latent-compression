#!/usr/bin/env python3
"""
Quick script to parse training logs and visualize progress.
Usage: python monitor_training.py < training.log
"""
...
def parse_training_log(lines: List[str]) -> Tuple[Dict[int, List[float]], Dict[int, Dict[str, float]]]:
    """Parse training log lines for losses and eval metrics."""
    
    epoch_losses = {}  # epoch -> [total_losses]
    eval_metrics = {}   # epoch -> {metric: value}
    ...
    if len(eval_metrics) >= 2:
        ...
        if last_epoch in eval_metrics and prev_epoch in eval_metrics:
            if 'em' in eval_metrics[last_epoch] and 'em' in eval_metrics[prev_epoch]:
                ...
                if em_improvement < 0.01:  # Less than 1% improvement
